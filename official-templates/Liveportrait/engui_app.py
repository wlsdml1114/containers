...
def process_video(video_file, workspace_dir, frame_number, image, region_input):
    # 워크스페이스 디렉토리 설정
    ...
    for segment in tqdm(range(num_segments)):
        ...
        output_file = os.path.join(split_folder, f"out_{segment}.mp4")
        added_output_file = os.path.join(added_folder, f"{segment}.mp4")
    ...
    #     python_path = f"/venv/bin/python"  # Python 경로 (필요시 절대 경로로 수정)
    ...
    if image is not None:
        ...
        # motion_template.py를 실행할 경로 설정
        python_path = f"/venv/bin/python"  # Python 경로 (필요시 절대 경로로 수정)
        script_path = f"{current_dir}/inference.py"  # motion_template.py 스크립트 경로 (필요시 절대 경로로 수정)
...
def create_interface():
    with gr.Blocks() as demo:
        ...
        with gr.Tab("Video Processor"):
            ...
            with gr.Group():
                gr.Markdown("#### Options")
                region_input = gr.Radio(label="Select Region for Smoothing", choices=['exp', 'eyes', 'lip', 'pose', 'all'], value='all')
            ...
            process_button.click(
                process_video,
                inputs=[video_input, workspace_input, frame_input, image_input, region_input],
                outputs=[result_text,output_video]
            )

        with gr.Tab("Video Processor from path"):
            gr.Markdown("### Video Processor with Motion Template Generator")
            ...
            with gr.Row():
                image_input = gr.Image(label="Upload Image", type="pil")
                video_input = gr.Textbox(label="Upload Video")
